"""
Script to analyze impact of sparsity on faithful of circuits.
"""
...
from llm_error_detection.circuits.circuit_discovery import (
    faithfulness_given_sparsity,
    learn_edge_scores,
    load_data,
    load_model,
)
...
SPARSITIES = [1e-5, 2e-5, 3e-5, 4e-5, 5e-5]
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="z1",
        choices=["z1", "z2", "both", "shortened_z1"],
...
def main() -> None:
    ...
    result_path = get_save_dir_name(
        prefix="results/sparsity-vs-faithfulness", template=args.template
    )
    ...
    for sparsity in SPARSITIES:
        logging.info(f"Compute faithfulness for sparsity: {sparsity}")
        n_edge, metric_value = faithfulness_given_sparsity(
            model=model,
        ...
        save_results(
            file_path=f"{result_path}/{args.metric}_{args.perturbation}.csv",
